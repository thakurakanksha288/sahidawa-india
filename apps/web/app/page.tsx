import { Camera, Mic, MapPin, Bell, History, Home, User, ShieldCheck, AlertTriangle, Globe, ChevronRight, Activity, Search } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur-lg">
        <div className="container mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-800">SahiDawa</h1>
          </div>
          
          <div className="flex items-center gap-4">
            <nav className="hidden md:flex items-center gap-6 text-sm font-semibold text-slate-600">
              <button className="hover:text-emerald-600 transition-colors">How it Works</button>
              <button className="hover:text-emerald-600 transition-colors">Alerts</button>
              <button className="hover:text-emerald-600 transition-colors">Pharmacy Map</button>
            </nav>
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            <div className="space-y-4 max-w-2xl">
              ...
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 tracking-tight leading-[1.1]">
                Your Health, <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500">Verified & Protected.</span>
              </h2>
              <p className="text-lg md:text-xl text-slate-500 font-medium leading-relaxed max-w-xl">
                India's open-source citizen platform to instantly verify medicine authenticity, find safe pharmacies, and receive critical health alerts.
              </p>
            </div>

            {/* Primary Action - Scan Barcode */}
            <button className="w-full sm:w-auto min-w-[300px] group relative overflow-hidden rounded-[2rem] bg-emerald-600 text-white p-8 shadow-xl shadow-emerald-600/20 transition-all active:scale-[0.98] hover:shadow-emerald-600/40 border border-emerald-500 text-left flex items-center justify-between">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <button className="flex items-center gap-5 bg-white border border-slate-200 p-6 rounded-3xl active:scale-95 transition-all group hover:border-blue-200 hover:shadow-lg hover:shadow-blue-100/50 text-left">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-0.5 font-medium leading-snug">Speak symptoms in your local language</p>
                </div>
              </button>

              <button className="flex items-center gap-5 bg-white border border-slate-200 p-6 rounded-3xl active:scale-95 transition-all group hover:border-amber-200 hover:shadow-lg hover:shadow-amber-100/50 text-left">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-sm mt-0.5 font-medium leading-snug">Find verified medical stores near you</p>
                </div>
              </button>
            </div>
          </div>
